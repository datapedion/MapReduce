/*
 * Map and Reduce Paradigm - Single
 */
...
public abstract class GenericMapReduce<T extends GenericMapReduce<T, K, V>, K, V> {
	...
	/**
	 * On all elements of the input stream the mapper is applied, grouping the
	 * stream by the key (collect) and reducing the result by the reducer.
	 *
	 * @param input
	...
	public T mapAndReduce( Stream<String> input, Function<String, Stream<Pair<K, V>>> mapper,
		...
		input.flatMap( mapper )
		     .collect(Collectors.groupingBy( Pair<K, V>::getKey, Collectors.mapping( Pair::getValue, Collectors.toList() ) ) )
